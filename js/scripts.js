...
if (document.querySelector('.shop-page')) {
    ...
    $('.range__line').slider({
        ...
        stop: function(event, ui) {
            $("#priceFrom").val($('.range__line').slider('values', 0));
            $("#priceTo").val($('.range__line').slider('values', 1));
...
console.log(searchForm);
    searchForm.addEventListener('submit', (evt) =>{
        ...
        console.log(url);
        $.post("/filter.php", url, function (json) {
            $(".shop__list").html('');
            $.each(json.products, function (index, value) {
                $(".shop__list").append('<article class="shop__item product" tabindex="0">' +
                                            '<div class="product__image">' +
                                                '<img src="/img/products/' + value.imgName + '"' + ' alt="product-name">' +
                                            '</div>' +
                                            '<p class="product__name">' + value.name + '</p>' +
                                            '<span class="product__price">' + value.price + ' руб.</span>' +
                                        '</article>')
            });
            $(".shop__paginator").html('');
            var page = 1;
            for (page = 1; page <= json.pages; page++) {
                $(".shop__paginator").append('<li><a class="paginator__item" href="?'+ url + '&' + 'page=' + page + '">' + page + '</a></li>');
            }

            $(".res-sort").html('');
            var total = json.total;
            $(".res-sort").append(total);
        }, "json");
